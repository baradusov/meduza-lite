...
const prepareStaticTweet = async (data) => {
  ...
  if (oEmbedTweet.error) {
    return `<p>Закрытый аккаунт или твит удалён, <a href="${twitterUrl}">посмотреть в Твиттере</a>.</p>`;
  }
...
export const prepareStaticTelegramPost = async (data) => {
  ...
  const image = bgImageStyle.split("'")[1];
...
export const withStaticEmbeds = async (data) => {
  ...
  await Promise.all([
    ...
    ...contentData.map(replaceWithStaticInstagramPosts),
    ...contentData.map(replaceWithStaticTelegramPosts),
    ...contentData.map(replaceWithStaticTiktoks),
  ]);
