...
export const getTweet = async (url) => {
  const response = await fetch(`${HOST}/api/tweet?url=${encodeURIComponent(url)}`);
...
export const getTiktok = async (url) => {
  const response = await fetch(`${HOST}/api/tiktok?url=${encodeURIComponent(url)}`);
  const data = await response.json();
...
export const getInstagramPost = async (url) => {
  const response = await fetch(`${HOST}/api/instagram?url=${encodeURIComponent(url)}`);
  const data = await response.json();
...
export const prepareStaticInstagramPost = async (data) => {
  ...
  const { thumbnail_url: imageUrl, author_name: author } =
